...
Work proceeds through evidence gates. Parallel research starts only after contracts stabilize.

1. Preserve current API, baseline artifacts, provenance, and missing numeric locks.
2. Stabilize configuration, component, plan, lock, source, measurement, and assurance schemas.
3. Add typed representations, conversions, material lifetimes, and compatibility diagnostics around
   existing kernels without changing baseline numerical behavior.
4. Move remaining hot-path allocation, transport, scheduling, measurement, and model processing to
   coarse Rust interfaces; verify wheel and separated-role parity.
5. Connect source-locked methods, immutable lineage, view-restricted attacks, formal scopes, and the
   matched multi-objective benchmark launcher.
