...
| [`dev dashboard`](/cli/reference/dev/dashboard/) | Experimental | Uses the shared `LocalTopology` to run the two remote roles on one machine and records local diagnostic history, not canonical research evidence |
| [`benchmark run`](/cli/reference/benchmark/run/) | Experimental | Runs the dashboard driver in-process over the same two role processes and writes a sanitized diagnostic report, not a canonical `EvidenceReport` |
...
| Component family classes | Available for all previously registered compiler components plus current guarded/blinded/secure/direct-FHE/cleartext linear arms, BFV and HE-authenticated preparation, seeded correlation expansion, inference role, client-local KV state, linear integrity, and tiny/bundle model sources; descriptors are class-derived |
| Typed pipeline profiles | `MaskedLinearCpu` is available as the two-role public-weight baseline; `ProprietaryGuarded`, `ProprietaryBlinded`, and `DirectFHEProfile` are available as one-role proprietary runtime profiles through the same configuration, local topology, gateway, and diagnostic benchmark paths; `SecureLinear`, `BFVCorrelations` as a distinct profile, plaintext reference backends, and protected research profiles remain uncomposed |
| Provider metadata discovery | Available through static `pllm.providers.v1` manifests with distribution/resource hash checks, package confinement, host/category version checks, editable-install opt-in, and host-generated external component classes; Python factory import requires an explicit provider approval set; Native Plugin ABI 1 and its conformance fixtures are available, while runtime native-library loading remains unavailable |
| Metric contracts and evidence registry | Typed latency, throughput, communication, memory, energy, accuracy, perplexity, and cost components are available; `BenchmarkResult` validates canonical full-result records and `EvidenceRegistry` retains and queries exact completed, failed, and unavailable cohorts; existing native region/deployment reports and dashboard diagnostics remain their documented schemas rather than being silently relabeled |
| Typed search | Exhaustive `GridSearch`, seeded discrete `RandomSearch`, path constraints, result binding, and explicit-direction `ParetoFrontier` are available over immutable Experiments and canonical benchmark results; failed/unavailable results remain recorded but unranked; Bayesian search and CLI search/compare remain unavailable |
| Research registry and quarantine | Canonical `SourceRecord`, `ArtifactLock`, `MethodRecord`, and `ResearchRegistry` contracts are available; existing source records and the MPCache upstream lock validate, restricted artifacts remain external-process/oracle-only, and promotion fails closed across source, reproduction, native implementation, lifecycle, and profile gates; the registry does not acquire or execute artifacts |
| Local role topology | Available through `build_roles`/`serve_local` for typed `Model`, `Pipeline`, or `Experiment` inputs; the public baseline keeps inference and preparation as separate child processes, while proprietary profiles start inference only; credentials stay out of argv/status, health and client routing are bound, and close is deterministic; this is loopback development orchestration, not operator separation |
...
| Dense-Qwen2 runtime schedule | Available for the model-aware prepared runtime; topology-derived q/k/v and gate/up fusion, all layers, final norm, output head, decoder tail, and prefill/decode state are bound to the model plan, runtime bundle, tokenizer, and quantized-weight digests; a plan-bound session enforces greedy selection, feedback, workload limits, per-stage output contracts, and poisoned-state failure handling; the schedule is client-heavy and not protected execution of the fixed-Q10 research composites |
...
| Decoder tail regions | Available for dense Qwen: prefill last-valid selection, decode physical-last selection, signed-wrap32 greedy selection with lowest-index tie-breaking, and token-id feedback; they participate in the complete model-aware baseline schedule but remain uncomposed in the fixed-Q10 research profile |
...
| Prepared public-weight generation | Available for runtime-supported text checkpoint layouts; validated end to end with `Qwen/Qwen2.5-0.5B-Instruct`, including the digest-bound baseline `ModelPlan` schedule and shared runtime weight bundle |
