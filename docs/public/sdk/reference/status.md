...
| Typed pipeline profiles | `MaskedLinearCpu` is available as the category-checked `baseline.masked_linear_cpu` slot contract with immutable whole-slot/nested updates; unresolved same-category alternatives remain rejected by native resolution and local serving until their full profiles are composed |
