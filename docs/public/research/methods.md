...
A method that resembles a paper is not automatically a faithful reproduction. `MethodRecord` keeps classification, semantic scope, source/artifact links, native ownership, implementation level, assurance obligations, and five lifecycle statuses separate. `ResearchRegistry.promotion()` permits a native operator/region/full model only when full text and artifact locks are resolved, upstream reproduction evidence is recorded, fidelity/benchmark/promotion pass, protected execution and assurance pass or are explicitly not applicable, and at least one profile is eligible. Structural adaptations and planned/reference-only methods remain blocked.
