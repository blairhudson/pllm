...
Use a Responses-compatible client with automatic offline preparation.
...
`OpenAI()` reads saved settings. The SDK sizes and prepares missing inventory
before the response enters its online phase; preparation never joins online
execution. Keep one client open to reuse unreserved inventory. A request-level
`model` overrides the saved default.
...
new client. Each response transparently ensures its required prepared capacity.
...
Automatic preparation occurs before online execution, but applications may move
that latency earlier by calling `prepared_rows_for_response()` and `preprocess()`
while idle. This is an optional warming control, not a correctness requirement.
