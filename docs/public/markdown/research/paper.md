...
The full paper. Preparation, decoding, and the client and provider boundary.


**Blair Hudson · September 2026 · Documentation edition 02**
...
This edition adds the application and deployment boundary. It uses the recorded lifecycle experiments; it does not introduce a new inference benchmark.

## Abstract

Private language model inference must account for preparation, network transfer, client computation and discarded work, not only the speed of a server kernel. We study a protocol that prepares fresh additive masks with homomorphic encryption and uses ordinary modular matrix multiplication during inference. We complete a BFV coefficient matrix backend through ciphertext reconstruction and client decryption. A matched 768 by 256 preparation stage becomes 4.93 times faster. We then execute complete dense stages at Qwen3.5 27B dimensions, including a 5,120 by 34,816 expansion, and measure the full 248,320 row output projection. A separate trained small decoder runs across isolated client and server processes from an empty preparation inventory. Its generated tokens match the clear quantized reference. Proposal verification reduces private calls by 60 percent but is slower at low latency because rejected proposals consume more preparation. Complete resource accounting for the 27B graph identifies communication and client cryptography as primary constraints. The work provides an executable inference trace and a capacity model. A local Responses interface separates application text from the provider protocol. This paper does not report generation from a loaded 27B checkpoint.
...
For a stage with integer matrix `W in Z^(m x n)` and activation `x`, choose an arithmetic modulus `p` and a fresh uniform mask `r in Z_p^n`. During preparation the client sends `Enc(r)` and the server computes `Enc(Wr)`. The client decrypts and retains `(r,Wr)`. During inference it sends `d=x+r mod p`, receives `u=Wd mod p`, and recovers
...
For `B` future masks with `B <=  N`, encode coordinate `i` as a polynomial

```
P_i(X) = sum(b = 0 to B - 1) r_b,i X^b
```

The server evaluates `Q_j = sum_i W_j,i Enc(P_i)`. Coefficient `b` of the decrypted result is exactly the `j`th coordinate of `Wr_b mod p`. Scalar linear combinations never mix polynomial coefficients. Thus the evaluator needs no rotations, rotation keys, bootstrapping, or products of two ciphertexts. All masks packed together use the same encryption key. Unrelated customer keys cannot be combined through ordinary batching.
...
A further arithmetic experiment keeps W4 weights but increases the activation range to `[-127,127]`. A width 17,408 dot product with all weights equal to `+7` or `-7` recovers the exact signed extrema `+/-15,475,712`. Using plaintext modulus 33,554,467 and the same ciphertext profile retains a minimum observed noise budget of 14 bits. The coefficient matrix program is unchanged.
...
```
T_k = (L_exchange + (k + 1) C_row + C_replay) / A_k
```

where `C_row` includes preparation and transfer, not only GEMM. Acceptance statistics should be measured by prefix, rather than assuming independent acceptance events. The executable cost selector is an experiment utility; it is not yet integrated into the public SDK’s scheduler.
...
A matrix kernel version of the recurrence is also checked against an elementwise reference. Its maximum FP32 output difference is `3.34 x 10^-6`; this is a floating summation difference, not bit equivalence. Such differences must be tested at quantization boundaries before deploying an existing checkpoint. Recent TreeWY research independently removes speculative state snapshots through structured reconstruction for Gated DeltaNet hybrids [8](#reference-8). Our simpler sequential replay does not claim that contribution; it tests the value of reconstructing state without another private query.
...
1.  <span id="reference-1"></span> J. Fan and F. Vercauteren. Somewhat Practical Fully Homomorphic Encryption. Cryptology ePrint 2012/144, 2012. [Source](https://eprint.iacr.org/2012/144).
2.  <span id="reference-2"></span> Microsoft Research. Microsoft SEAL. Library source and serialization implementation, version 4.3.3. [Source](https://github.com/microsoft/SEAL).
3.  <span id="reference-3"></span> Qwen Team. Qwen3.5-27B: model configuration and model card. 2026. [Source](https://huggingface.co/Qwen/Qwen3.5-27B).
4.  <span id="reference-4"></span> Hugging Face contributors. Qwen3.5 model implementation, `modeling_qwen3_5.py`. Source inspected 6 September 2026. [Source](https://github.com/huggingface/transformers).
5.  <span id="reference-5"></span> S. Yang, J. Kautz and A. Hatamizadeh. Gated Delta Networks: Improving Mamba2 with Delta Rule. ICLR 2025. [Source](https://arxiv.org/abs/2412.06464).
6.  <span id="reference-6"></span> Y. Leviathan, M. Kalman and Y. Matias. Fast Inference from Transformers via Speculative Decoding. ICML 2023. [Source](https://arxiv.org/abs/2211.17192).
7.  <span id="reference-7"></span> Hugging Face. Assisted decoding: speculative and prompt lookup decoding. Transformers documentation. [Source](https://huggingface.co/docs/transformers/main/assisted_decoding).
8.  <span id="reference-8"></span> S. M. Ghantasala. TreeWY: Speculative Verification for Gated DeltaNet Hybrids. arXiv:2608.20961, 2026.
9.  <span id="reference-9"></span> Qwen Team. Qwen3.5-35B-A3B: model configuration. 2026. [Source](https://huggingface.co/Qwen/Qwen3.5-35B-A3B).
10. <span id="reference-10"></span> E. Boyle, G. Couteau, N. Gilboa, Y. Ishai, L. Kohl, P. Rindal and P. Scholl. Efficient Two-Round OT Extension and Silent Non-Interactive Secure Computation. Cryptology ePrint 2019/1159. [Source](https://eprint.iacr.org/2019/1159).
11. <span id="reference-11"></span> Z. Huang, W. Lu, C. Hong and J. Ding. Cheetah: Lean and Fast Secure Two-Party Deep Neural Network Inference. USENIX Security 2022, pp. 809–826.
12. <span id="reference-12"></span> Open Responses contributors. Open Responses Specification. Accessed 6 September 2026. [Source](https://www.openresponses.org/specification).
