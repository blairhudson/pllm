...
GitHub checks, PyPI publication and Pages deployment.
...
git add uv.lock docs/package-lock.json
...
`dependency-locks` artifact and commit both files in a PR. That workflow has no
...
both committed locks. Python's bounded library requirements remain in the wheel;
...
The tag must match the sole version declaration in `python/pllm/_version.py`.
