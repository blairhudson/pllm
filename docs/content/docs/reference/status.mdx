...
| Compiler coverage report | Available and composition-scoped; the `MaskedLinearCpu` preset composition is complete for untransformed dense Qwen2 through the model-aware runtime schedule, while whole-model protected composition remains incomplete |
