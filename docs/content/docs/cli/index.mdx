...
description: "Inspect PLLM records and run local benchmarks from the terminal."
---
...
This installs the `pllm.run` distribution from PyPI in an isolated tool
environment. Use `uv tool upgrade pllm.run` to update it. The installed command
remains `pllm`.
