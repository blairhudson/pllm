...
def test_model_component_transforms_generic_decoder_plan_immutably() -> None:
    ...
    assert base.to_dict()["transformations"] == []
    transformation = optimized.to_dict()["transformations"][-1]
    ...
    assert all(
        operation["layer"] is not None
        for operation in optimized.prefill["operations"]
        if operation["id"].startswith("method.mpcache.layer.")
    )
    assert {
        state["kind"] for state in optimized.prefill["state_outputs"]
    } == {"key", "value"}
